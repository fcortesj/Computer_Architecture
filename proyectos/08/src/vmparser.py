...
class Parser:
    ...
    def command_type(self):
        ...
        #Splits the command int parts to get the first one to check what commands is bewteen push, pop, label, goto and if-goto
        ...
        if partsCommand[0] in Parser.arithmetic_commands:
            ...
            return "C_POP"
        elif partsCommand[0] == "label":
            return "C_LABEL"
        elif partsCommand[0] == "goto":
            return "C_GOTO"
        elif partsCommand[0] == "if-goto":
            return "C_IF"
        elif partsCommand[0] == "call":
            return "C_CALL"
        elif partsCommand[0] == "function":
            return "C_FUNCTION"
        elif partsCommand[0] == "return":
            return "C_RETURN"
        else:
    ...
    def arg1(self):
        ...
        #Then we check if the commands are label creation, goto, if-goto, call, function or return
        elif commandType == "C_LABEL" or commandType == "C_GOTO" or commandType == "C_IF" or commandType == "C_CALL" or commandType == "C_FUNCTION" or commandType == "C_RETURN":
        #We check if the arg1 actually exists with the label, goto and if-goto
            try:
                return partsCommands[1]
            except IndexError:
                print("ERROR: There is no argument with command {}".format(partsCommands[0]))
    ...
    def arg2(self):
        ...
        partsCommand = currentCommand.split(" ")
        commandType = self.command_type()
        #We check the command type this only works with pop and push, or in other cases if the command is call or a function. Which return the number of args and local variables respectly
        if commandType == "C_PUSH" or commandType == "C_POP" or commandType == "C_CALL" or commandType == "C_FUNCTION":
