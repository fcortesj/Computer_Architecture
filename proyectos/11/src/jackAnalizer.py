...
def main(argv):
    ...
    if os.path.isdir(filepath):
        ...
        if justFile[-1] != "" and justFile[-1] != " ":
            searchFileDir(filepath, symbolTable)
        else:
            searchFileDir(filepath, symbolTable)
    else:
        #If it is a file then we just pass straight
        translateFile(filepath, symbolTable)



def searchFileDir(path, symbolTable):
    ...
    if len(files) >= 1:
        ...
        for jackFiles in files:
            translateFile(os.path.join(r,jackFiles), symbolTable)
    else:
...
def translateFile(filepath, symbolTable):
    ...
    vmTranslator = jackVMWriter(output)
    ...
    listener = jackListener(parser, vmTranslator, symbolTable)
